...
{
    const VERSION = '3.0.4';
    const USER_AGENT_ORIGIN = 'Jeeb PHP Library';
    ...
    public static function request($url, $method = 'POST', $params = array(), $authentication = array())
    {
        ...
        if (empty($auth_token)) {
            \Jeeb\Exception::throwException(400, array('reason' => 'AuthTokenMissing'));
        }
        ...
        if (!in_array($environment, $environments)) {
            ...
            \Jeeb\Exception::throwException(400, array(
                'reason' => 'BadEnvironment',
                'message' => "Environment does not exist. Available environments: $availableEnvironments"
            ));
        }

        $url = ($environment === 'sandbox' ? 'https://api-sandbox.jeeb.com/v2' : 'https://api.jeeb.com/v2')
            . $url;
        $headers = array();
        ...
        if ($http_status === 200) {
            ...
            \Jeeb\Exception::throwException($http_status, $response);
        }
    }
}
